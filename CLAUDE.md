...
- The frontend upserts a cell via `onConflict: 'periodo_id,dia,param_id'`; clearing a value issues a `DELETE` rather than an upsert with a null. Every write reports its outcome per cell (`guardado` state in `MonitoreoOsmosisInversa.jsx`: `guardando`/`ok`/`error`) — the local `registros` state is optimistic, so without that indicator a write that never landed still looks captured on screen.
